...
public class StartInput : MonoBehaviour
{
    ...
    [SerializeField] private string bashWorkingDirectory = "Robot-imitation-learning/";
    ...
    /// <summary>
    /// Fill the RoboList dropdown with all supported robot names + scene-only
    /// entries, so the user doesn't need to maintain staticOptions manually.
    /// </summary>
    private void PopulateRoboListDropdown()
    {
        if (roboListDropdown == null) return;

        var options = new List<string>();

        // 1) All WHAM/GMR-supported robot names
        foreach (string key in SupportedRobotNames)
            options.Add(key);

        // 2) Scene robots that have no supported-key mapping yet
        if (sceneRobots != null)
        {
            foreach (var entry in sceneRobots)
            {
                if (!string.IsNullOrWhiteSpace(entry.label) && !options.Contains(entry.label))
        ...
        if (options.Count == 0)
        {
            options.Add("(no robots found)");
        }

        int prevValue = roboListDropdown.value;
        string prevText = (roboListDropdown.options != null && prevValue >= 0 && prevValue < roboListDropdown.options.Count)
            ? roboListDropdown.options[prevValue].text : null;

        roboListDropdown.ClearOptions();
        roboListDropdown.AddOptions(options);

        // Restore previous selection if possible
        if (!string.IsNullOrEmpty(prevText))
        {
            int newIdx = options.FindIndex(o => string.Equals(o, prevText, System.StringComparison.OrdinalIgnoreCase));
            if (newIdx >= 0)
            {
                roboListDropdown.value = newIdx;
                roboListDropdown.RefreshShownValue();
            }
        }

        // Ensure the FileBrowser on the same object doesn't overwrite us on Start()
        var fb = roboListDropdown.GetComponent<FileBrowser>();
        if (fb != null)
        {
            fb.dropdownMode = FileBrowser.DropdownMode.StaticList;
            fb.staticOptions = options;
        }
    }
    ...
    private string ResolveWorkingDirectoryPath()
    {
        if (string.IsNullOrWhiteSpace(bashWorkingDirectory))
        {
            ...
            Debug.LogError("请在 Inspector 中设置 bashWorkingDirectory（相对路径会从 Assets/Imitation/ 或 Assets/Imitation/ 解析）。");
            return string.Empty;
        }
