...
SMPL_USERNAME=${SMPL_USERNAME:-}
SMPL_PASSWORD=${SMPL_PASSWORD:-}

if [[ -z "${SMPL_USERNAME}" ]]; then
    read -r -p "SMPL account email: " SMPL_USERNAME
fi
if [[ -z "${SMPL_PASSWORD}" ]]; then
    read -r -s -p "SMPL account password: " SMPL_PASSWORD
    echo
fi

if [[ -z "${SMPL_USERNAME}" || -z "${SMPL_PASSWORD}" ]]; then
    echo "SMPL credentials are required. Set SMPL_USERNAME and SMPL_PASSWORD or run interactively." >&2
    exit 2
fi
...
wget --post-data "username=${SMPL_USERNAME}&password=${SMPL_PASSWORD}" 'https://download.is.tue.mpg.de/download.php?domain=smplify&resume=1&sfile=mpips_smplify_public_v2.zip' -O './dataset/body_models/smplify.zip' --no-check-certificate --continue
...
wget --post-data "username=${SMPL_USERNAME}&password=${SMPL_PASSWORD}" 'https://download.is.tue.mpg.de/download.php?domain=smpl&sfile=SMPL_python_v.1.0.0.zip' -O './dataset/body_models/smpl.zip' --no-check-certificate --continue
